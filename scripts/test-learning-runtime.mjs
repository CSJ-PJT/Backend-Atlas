...
assert(subjectiveDocument.getElementById('subjectiveDifficulty').textContent==='기초','the default mixed bank must begin with beginner questions');
assert(subjectiveDocument.getElementById('subjectiveHintPanel').hidden&&subjectiveDocument.getElementById('subjectiveExamplePanel').hidden,'learning aids must start collapsed');
subjectiveDocument.getElementById('subjectiveHintBtn').click();
assert(!subjectiveDocument.getElementById('subjectiveHintPanel').hidden&&subjectiveDocument.getElementById('subjectiveHintText').textContent.trim(),'hint control must reveal a useful hint');
assert(subjectiveDocument.getElementById('subjectiveGuide').hidden,'opening a hint must not reveal the answer');
subjectiveDocument.getElementById('subjectiveExampleBtn').click();
assert(!subjectiveDocument.getElementById('subjectiveExamplePanel').hidden&&subjectiveDocument.getElementById('subjectiveExampleText').textContent.trim(),'example control must reveal a practical example');
assert(subjectiveDocument.getElementById('subjectiveGuide').hidden,'opening an example must not reveal the answer');
subjectiveDocument.getElementById('subjectiveLevel').value='medium';
subjectiveDocument.getElementById('subjectiveLevel').dispatchEvent(new subjectiveDom.window.Event('change'));
assert(subjectiveDocument.getElementById('subjectiveDifficulty').textContent==='응용','difficulty filter must render only applied questions when selected');
assert(subjectiveDocument.getElementById('subjectiveHintPanel').hidden&&subjectiveDocument.getElementById('subjectiveExamplePanel').hidden,'changing filters must collapse prior learning aids');
subjectiveDocument.getElementById('subjectiveLevel').value='';
subjectiveDocument.getElementById('subjectiveLevel').dispatchEvent(new subjectiveDom.window.Event('change'));
assert(subjectiveDocument.getElementById('subjectiveDifficulty').textContent==='기초','returning to the mixed bank must restore beginner-first ordering');
...
assert(!subjectiveDocument.getElementById('subjectiveSource').hidden&&subjectiveDocument.getElementById('subjectiveSource').href.startsWith('https://'),'a correct answer must reveal its primary source');
